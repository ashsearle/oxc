...
use oxc_semantic::{AstNodes, JsDocTag, Scope, ScopeTree, Semantic, SemanticNode, SymbolTable};
...
impl<'a> LintContext<'a> {
    ...
    /* JsDoc */
    pub fn jsdoc(&self, node: &AstNode<'a>) -> Option<Rc<[JsDocTag<'a>]>> {
        self.semantic().jsdoc().get(node)
    }
}
